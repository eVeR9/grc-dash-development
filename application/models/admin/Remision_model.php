<?php
    ...
    function json_new() {
		$this->datatables->select('r.id, r.id_pedido as id_pedido, r.serie, r.id_serie, r.id as id_remision, r.fecha_remision, r.id_cliente, r.nombre_flete, r.orden_de_compra, r.salida_flete, r.tipo_flete, r.id_fase_de_remision,r.ticket_bascula, ped.toneladas_embarcadas, r.nombre_operador_flete,r.monto_total_remision,  r.placas_flete,rf.fase_de_remision, r.toneladas_remision,r.obra_cliente, ped.id_cliente, ped.razon_social,ped.toneladas_embarcadas, ped.toneladas,
		ped.id_vendedor, ped.nombre_vendedor,  ped.id_material, ped.nombre_del_material, ped.id_registro_patronal, ped.empresa_remisiona');
		$this->datatables->from('remisiones r');
		$this->datatables->join('remisiones_fases rf', 'r.id_fase_de_remision = rf.id', 'left');
		$this->datatables->join('(SELECT p.id, p.id_cliente, c.razon_social,p.toneladas_embarcadas, p.toneladas, p.id_vendedor, CASE p.id_vendedor WHEN 0 THEN "BASCULA" ELSE concat(u.nombre," ",u.apellidos) END as nombre_vendedor,  p.id_material, m.nombre_del_material, p.id_registro_patronal, rp.razon_social as empresa_remisiona FROM pedidos p LEFT join clientes c on p.id_cliente = c.id LEFT join usuarios u on p.id_vendedor = u.id LEFT join registro_patronal rp on p.id_registro_patronal = rp.id LEFT join materiales m on p.id_material = m.id) ped', 'r.id_pedido = ped.id', 'left');
    ...
    function json() {
		...
		$this->datatables->add_column('imprimir', '<a href="imprimir/$1" class="btn btn-info btn-flat">Imprimir</a>', 'id');
		if($depto == "MINA" || $depto == "TI") {
		$this->datatables->add_column('editar', '<a href="edit/$1" class="btn btn-info btn-flat">Editar</a>', 'id');
		} else {
		$this->datatables->add_column('editar', '<a href="#" class="btn btn-info btn-flat">Editar</a>');
		}
		$this->datatables->add_column('ver', '<a href="view/$1" class="btn btn-info btn-flat">Ver</a>', 'id');
		return $this->datatables->generate();
    }
 ...
 /* TODAS LAS REMISIONES */
   function get_all_remisiones() {
	...
	//update pedidos set toneladas_embarcadas = toneladas_embarcadas + 2000 where id_pedido=1	
		public function update_pedido_toneladas(){
			//$this->db->query("UPDATE `pedidos` SET `toneladas_embarcadas` = `toneladas_embarcadas` + $toneladas_remision WHERE `id` = $id_pedido");
			$qry = "UPDATE pedidos p INNER JOIN (SELECT id_pedido, SUM(toneladas_remision) as total FROM remisiones where id_fase_de_remision in (2,5,6) group by id_pedido) r ON p.id = r.id_pedido SET p.toneladas_embarcadas = r.total";
			$this->db->query($qry);
