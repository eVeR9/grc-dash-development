<?php
    ...
    function json() {
        ...
        $this->datatables->from('remisiones_espera r');
		...
		//$this->datatables->add_column('view', '<a href="espera/imprimir/$1">Imprimir</a> | <a href="espera/edit/$1">Editar</a> | <a href="espera/view/$1">Ver</a>', 'id');
		$depto = strtoupper($this->session->userdata('departamento'));

		$this->datatables->add_column('imprimir', '<a href="espera/imprimir/$1" class="btn btn-info btn-flat">Imprimir</a>', 'id');
		if($depto == "MINA" || $depto == "TI") {
		$this->datatables->add_column('editar', '<a href="espera/edit/$1" class="btn btn-info btn-flat">Editar</a>', 'id');
		} else {
		$this->datatables->add_column('editar', '<a href="#" class="btn btn-info btn-flat">Editar</a>');
		}
		$this->datatables->add_column('ver', '<a href="espera/view/$1" class="btn btn-info btn-flat">Ver</a>', 'id');
    ...
    public function get_datatables()
    {
        ...
        $this->db->from('remisiones_espera r');
 ...
 /* TODAS LAS REMISIONES */
   function get_all_remisiones() {
	...
	/* ULTIMO ID INSERTADO */
		...
		public function get_all_remisiones_complete(){
			$query = $this->db->query("r.id, r.id_pedido, r.fecha_remision, c.razon_social, r.nombre_flete, p.orden_de_compra, r.orden_de_compra as orden_de_compra_remision, r.salida_flete, r.tipo_flete, r.id_fase_de_remision, rf.fase_de_remision, r.id_vendedor, concat(u.nombre,' ',u.apellidos) as nombre_vendedor, r.ticket_bascula, r.toneladas_embarcadas, r.nombre_operador_flete, r.monto_total_remision,  r.placas_flete, p.obra_cliente, r.id_material, m.nombre_del_material, r.toneladas_remision, r.created_by, r.chk_crg, r.cargado_externo FROM remisiones_espera r LEFT JOIN clientes c ON  r.id_cliente = c.id LEFT JOIN usuarios u ON r.id_vendedor = u.id LEFT JOIN registro_patronal rp ON r.id_registro_patronal = rp.id LEFT JOIN materiales m ON r.id_material = m.id LEFT JOIN pedidos p ON r.id_pedido = p.id LEFT JOIN remisiones_fases rf ON r.id_fase_de_remision = rf.id ORDER BY r.id DESC");
			return $result = $query->result_array();
		}
		...
		public function get_all_pedidos_complete_id($id){
			...
			//$this->db->join('remisiones_espera r', 'p.id = r.id_pedido', 'left');
		...
		public function get_all_remisiones_fases_espera(){
			$query = $this->db->get('remisiones_fases_espera');
			return $result = $query->result_array();
		}
  ...
  public function get_remisiones_espera_by_id($id) {
	  $query = $this->db->get_where('remisiones_espera', array('id' => $id));
	  //$this->db->get();
  ...
  public function get_remisiones_espera(){
	  ...
	  //if($isBascula || $isVigilancia){
		$this->db->select('re.id, id_estatus_espera, res.fase_de_remision, fecha_espera, id_pedido, m.nombre_del_material, c.razon_social, nombre_flete, nombre_operador_flete, placas_flete, entrada_flete, destino_flete');
		...
		$this->db->join('remisiones_fases_espera res', 're.id_estatus_espera = res.id', 'left' );		
		$this->db->where('id_estatus_espera <', '3');
  ...
  public function cambiar_estatus_a_entrada($id){
	$qry = "UPDATE remisiones_espera SET id_estatus_espera = 2 WHERE id =".$id;
	$this->db->query($qry);
	return true;
  }

  public function cambiar_estatus_a_cancelar($id){
	  $qry = "UPDATE remisiones_espera SET id_estatus_espera = 3 WHERE id = ".$id;
	  $this->db->query($qry);
	  return true;
  }
