...
class Laboratorio_model extends CI_Model
{
    ...
    public function edit_barreno($data)
    {
        
        $this->db->insert('laboratorio_barrenos' ,$data);
        return true;
        //return $this->db->query('UPDATE barrenacion where check_analisis, $data = id, $id');
    }

    //Funcion del Check
    public function edit_barrenos($data2, $check_analisis)
    {
        
        $this->db->where('id', $check_analisis);
        $this->db->update('barrenacion', $data2);
    ...
    public function get_all_barrenacion_laboratorio()
    {
        $this->db->select('b.*,lb.* ,b.bar_perf');
        $this->db->from('barrenacion as b');
        $this->db->join('laboratorio_barrenos as lb','lb.id_barreno = b.bar_perf');
        $this->db->order_by('b.bar_perf', 'desc');
        $query = $this->db->get();
        return $result = $query->result_array();
    }
}
