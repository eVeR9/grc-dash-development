<?php
    ...
    function json() {
        $this->datatables->select('r.id, r.id_pedido as id_pedido, r.id as id_remision, r.fecha_remision, r.id_cliente, c.razon_social, r.nombre_flete, r.orden_de_compra, r.salida_flete, r.tipo_flete, r.id_fase_de_remision, rf.fase_de_remision, r.id_vendedor, CASE r.id_vendedor WHEN 0 THEN "Bascula" ELSE concat(u.nombre," ",u.apellidos) END as nombre_vendedor, r.ticket_bascula, r.toneladas_embarcadas, r.nombre_operador_flete, r.monto_total_remision,  r.placas_flete, r.obra_cliente, r.id_material, m.nombre_del_material, r.toneladas_remision, r.id_registro_patronal, rp.razon_social as empresa_remisiona, r.created_by, r.serie, r.id_serie, r.added_time');
        ...
        $this->db->order_by('r.added_time','desc');
		//$this->datatables->join('country', 'city.CountryCode = country.Code');
		//$this->datatables->add_column('view', '<a href="remisiones/imprimir/$1">Imprimir</a> | <a href="remisiones/edit/$1">Editar</a> | <a href="remisiones/view/$1">Ver</a>', 'id');
		$depto = strtoupper($this->session->userdata('departamento'));

		$this->datatables->add_column('imprimir', '<a href="remisiones/imprimir/$1" class="btn btn-info btn-flat">Imprimir</a>', 'id');
		if($depto == "MINA" || $depto == "TI") {
		$this->datatables->add_column('editar', '<a href="remisiones/edit/$1" class="btn btn-info btn-flat">Editar</a>', 'id');
		} else {
		$this->datatables->add_column('editar', '<a href="#" class="btn btn-info btn-flat">Editar</a>');
		}
		$this->datatables->add_column('ver', '<a href="remisiones/view/$1" class="btn btn-info btn-flat">Ver</a>', 'id');
		return $this->datatables->generate();
    }


    function json_espuela() {
        $this->datatables->select('r.id, r.id_pedido as id_pedido, r.id as id_remision, r.fecha_remision, r.id_cliente, c.razon_social, r.nombre_flete, r.orden_de_compra, r.salida_flete, r.tipo_flete, r.id_fase_de_remision, rf.fase_de_remision, r.id_vendedor, CASE r.id_vendedor WHEN 0 THEN "Bascula" ELSE concat(u.nombre," ",u.apellidos) END as nombre_vendedor, r.ticket_bascula, r.toneladas_embarcadas, r.nombre_operador_flete, r.monto_total_remision,  r.placas_flete, r.obra_cliente, r.id_material, m.nombre_del_material, r.toneladas_remision, r.id_registro_patronal, rp.razon_social as empresa_remisiona, r.created_by, r.serie, r.id_serie');
        $this->datatables->from('remisiones r');
		$this->datatables->where('r.serie', 'E');
      	$this->datatables->join('clientes c', 'r.id_cliente = c.id', 'left');
      	$this->datatables->join('usuarios u', 'r.id_vendedor = u.id', 'left');
      	$this->datatables->join('registro_patronal rp', 'r.id_registro_patronal = rp.id', 'left');
      	$this->datatables->join('materiales m', 'r.id_material = m.id', 'left');
 ...
 /* TODAS LAS REMISIONES */
   function get_all_remisiones() {
	...
	/* ACTUALIZAR TONELADAS DEL PEDIDO */
	//update pedidos set toneladas_embarcadas = toneladas_embarcadas + 2000 where id_pedido=1	
		public function update_pedido_toneladas($id_pedido){
			//$this->db->query("UPDATE `pedidos` SET `toneladas_embarcadas` = `toneladas_embarcadas` + $toneladas_remision WHERE `id` = $id_pedido");
			$this->db->query("UPDATE `pedidos` p INNER JOIN (SELECT id_pedido, SUM(toneladas_remision) as total FROM remisiones where `id_pedido` = $id_pedido and id_fase_de_remision in (2,5,6)) r ON p.id = r.id_pedido SET p.toneladas_embarcadas = r.total");
			
			//cargado, cargado interno y entregado interno

			//$this->db->set('toneladas_embarcadas', 'toneladas_embarcadas'+ $data, FALSE);
			//$this->db->where('id', $id);
	...
	/* ULTIMO ID INSERTADO */
		...
		public function get_all_remisiones_complete(){
			$query = $this->db->query("r.id, r.id_pedido, r.fecha_remision, c.razon_social, r.nombre_flete, p.orden_de_compra, r.orden_de_compra as orden_de_compra_remision, r.salida_flete, r.tipo_flete, r.id_fase_de_remision, rf.fase_de_remision, r.id_vendedor, concat(u.nombre,' ',u.apellidos) as nombre_vendedor, r.ticket_bascula, r.toneladas_embarcadas, r.nombre_operador_flete, r.monto_total_remision,  r.placas_flete, p.obra_cliente, r.id_material, m.nombre_del_material, r.toneladas_remision, r.created_by, r.chk_crg, r.cargado_externo FROM remisiones r LEFT JOIN clientes c ON  r.id_cliente = c.id LEFT JOIN usuarios u ON r.id_vendedor = u.id LEFT JOIN registro_patronal rp ON r.id_registro_patronal = rp.id LEFT JOIN materiales m ON r.id_material = m.id LEFT JOIN pedidos p ON r.id_pedido = p.id LEFT JOIN remisiones_fases rf ON r.id_fase_de_remision = rf.id ORDER BY r.id DESC");
			return $result = $query->result_array();
		}

		public function get_all_remisiones_complete_id($id){
    		$this->db->select('r.id, r.id_pedido, r.id_cliente, r.fecha_remision, r.serie, r.id_serie, c.razon_social, r.nombre_flete, p.orden_de_compra, r.orden_de_compra as orden_de_compra_remision, r.salida_flete, r.tipo_flete, r.destino_flete, r.cajas_flete, r.id_fase_de_remision, rf.fase_de_remision, r.id_vendedor, concat(u.nombre," ",u.apellidos) as nombre_vendedor, r.ticket_bascula, r.peso_inicial, r.peso_final, r.toneladas_embarcadas, r.nombre_operador_flete, r.monto_total_remision,  r.placas_flete, r.entrada_flete, p.obra_cliente, r.obra_cliente as obra_cliente_remision, r.id_material, m.nombre_del_material, r.toneladas_remision, r.id_sucursal, cs.sucursal, r.created_by, p.fecha_pedido, p.orden_de_compra, r.orden_de_compra as orden_de_compra_remision, p.orden_de_compra, p.toneladas, p.toneladas_embarcadas, p.precio_tonelada, p.monto_total, re.razon_social as razon_social_re, p.id_registro_patronal, p.notas, r.notas_remision, r.chk_crg, r.id_registro_patronal, r.cargado_externo, venta_directa');
			$this->db->from('remisiones r');
  ...
  public function add_remision_espera($data){
	$this->db->insert('remisiones_espera', $data);
	return true;
}

public function get_remision_espera($id) {
	$this->db->select('re.*, p.orden_de_compra, p.razon_social, p.id_cliente, p.razon_social_re, p.id_registro_patronal, p.nombre_del_material, p.id_material, p.nombre_sucursal, p.id_suc, p.obra_cliente, p.id_vendedor, p.nombre_vendedor, p.precio_tonelada, p.monto_total, p.toneladas, p.toneladas_embarcadas, p.toneladas_diferencia, p.serie, p.notas');
	$this->db->where('re.id', $id);
	//$this->db->where('re.id_estatus_espera <','3');
	$this->db->from('remisiones_espera re');
	$this->db->join('
	(SELECT p.*, m.nombre_del_material as nombre_del_material, c.razon_social, cs.sucursal as nombre_sucursal, cs.id as id_suc, rp.razon_social as razon_social_re, concat(u.nombre," ",u.apellidos) as nombre_vendedor 
	FROM pedidos p left join materiales m on p.id_material = m.id left join registro_patronal rp on p.id_registro_patronal = rp.id 
	left join clientes c on p.id_cliente = c.id left join clientes_sucursal cs on p.id_sucursal = cs.id 
	left join usuarios u on p.id_vendedor = u.id ) as p', 're.id_pedido = p.id', 'left');
	//$this->db->join('materiales m', 're.id_material = m.id', 'left' );
	//$this->db->join('pedidos p', 're.id_pedido = p.id', 'left' );	
	//$this->db->join('registro_patronal rp', 're.id_registro_patronal = rp.id', 'left' );
	$query = $this->db->get();
  ...
  public function get_remision_espera_by_id($id) {
	$this->db->select('re.*, p.orden_de_compra, p.razon_social, p.id_cliente, p.razon_social_re, p.id_registro_patronal, p.nombre_del_material, p.id_material, p.nombre_sucursal, p.id_suc, p.obra_cliente, p.id_vendedor, p.nombre_vendedor, p.precio_tonelada, p.monto_total, p.toneladas, p.toneladas_embarcadas, p.toneladas_diferencia, p.serie, p.notas');
	$this->db->where('re.id', $id);
	$this->db->from('remisiones_espera re');
	$this->db->join('(SELECT p.*, m.nombre_del_material as nombre_del_material, c.razon_social, cs.sucursal as nombre_sucursal, cs.id as id_suc, rp.razon_social as razon_social_re, concat(u.nombre," ",u.apellidos) as nombre_vendedor FROM pedidos p left join materiales m on p.id_material = m.id left join registro_patronal rp on p.id_registro_patronal = rp.id left join clientes c on p.id_cliente = c.id left join clientes_sucursal cs on p.id_sucursal = cs.id left join usuarios u on p.id_vendedor = u.id ) as p', 're.id_pedido = p.id', 'left');
	//$this->db->join('materiales m', 're.id_material = m.id', 'left' );
	//$this->db->join('pedidos p', 're.id_pedido = p.id', 'left' );	
	//$this->db->join('registro_patronal rp', 're.id_registro_patronal = rp.id', 'left' );
	$query = $this->db->get();
...
/*
  public function get_remisiones_espera(){
	  ...
	  if($isBascula || $isVigilancia){
		$this->db->select('re.id, estatus, id_espera, fecha_espera, id_pedido, m.nombre_del_material as id_material, c.razon_social as id_cliente, nombre_flete, nombre_operador_flete, placas_flete, entrada_flete, destino_flete');
		$this->db->from('remisiones_espera re');
		$this->db->join('clientes c', 're.id_cliente = c.id', 'left');
		$this->db->join('materiales m', 're.id_material = m.id', 'left' );
		$this->db->like('estatus', 'Entrada');
		$this->db->or_like('estatus', 'Espera');
		//$this->db->like();
		$query = $this->db->get();
	  ...
	  /*
	  $query = $this->db->get('remisiones_espera');
	  $result = $query->result_array();
	  return $result;
	  */
  //}

  public function camiones_espera(){
	$this->db->select('*');
	$this->db->from('remisiones_espera');
	$this->db->like('estatus', 'Entrada');
	$query = $this->db->get();
	$result = $query->result_array();
	return $result;
  ...
  public function cambiar_estatus_a_entrada($id){
	$qry = "UPDATE remisiones_espera SET estatus = 'Entrada' WHERE remisiones_espera . id =".$id;
	$this->db->query($qry);
	return true;
  }

  public function cambiar_estatus_a_cancelar($id){
	  $qry = "UPDATE remisiones_espera SET estatus = 'Cancelada' WHERE remisiones_espera . id = ".$id;
	  $this->db->query($qry);
	  return true;
  }
