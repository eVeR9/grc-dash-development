...
class Laboratorio extends MY_Controller
{
    ...
    public function edit_barrenos($id = 0, $check_analisis = 0)
    {
        ...
        if ($update) {
            ...
            $this->form_validation->set_rules('cao', 'cao', 'trim|required');
            $this->form_validation->set_rules('comentario', 'comentario', 'trim|required');
            $this->form_validation->set_rules('check_analisis', 'check_analisis', 'trim|required');

            $runUpdate = $this->form_validation->run() == FALSE;
            if ($runUpdate) {
                $data['laboratorio_estatus'] = $this->laboratorio_model->getestatus();
                $data['laboratorio_maquina'] = $this->laboratorio_model->get_maquina();
                ...
                $data = array(
                 ...
                 $data2 = array(
                 'check_analisis' => $this->input->post('check_analisis')
                 );

                 $check_analisis = $this->input->post('check_analisis');
               
                $data = $this->security->xss_clean($data);
                $result = $this->laboratorio_model->edit_barreno($data);


                if ($result) {
                    $this->laboratorio_model->edit_barrenos($data2, $check_analisis);
                    $this->session->set_flashdata('msg', 'Registro Actualizado!');
                    redirect(base_url('admin/laboratorio/bitacora_laboratorio_barrenos'));
                }
            }
    ...
    //Funcion para mostrar datos en la bitacora desde tabla Barrenacion
