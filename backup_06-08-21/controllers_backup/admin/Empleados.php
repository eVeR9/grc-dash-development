<?php
	...
	class Empleados extends MY_Controller {
		...
		public function subir_fotografia($numero_empleado, $extension, $archivo) //variable $id_empleado 
        {			
                ...
                if ( ! $this->upload->do_upload($archivo))
                	{		//si no se pudo subir
						echo $new_name;
						echo $this->upload->display_errors();
						$this->session->set_flashdata('msg', 'Error al subir archivo'); //variable $campo concatenada
						redirect(base_url('admin/empleados'));
                	}
                else
                	{		// si se pudo subir
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$data = array(
						...
						'salario_neto_diario' => $this->input->post('salario_neto_diario'),
